...
class FivetranOperator(BaseOperator):
    ...
    :param schedule_type: Fivetran connector's schedule_type, Default is manual, to take connector off Fivetran schedule. Set to auto and keep connector on Fivetran schedule
...
class FivetranResyncOperator(BaseOperator):
    """
    `FivetranResyncOperator` starts a Fivetran historical resync. https://fivetran.com/docs/rest-api/connectors#resyncconnectortabledata

    `FivetranResyncOperator` requires that you specify the `connector_id` of the sync job to
    start. You can find `connector_id` in the Settings page of the connector you
    configured in the `Fivetran dashboard <https://fivetran.com/dashboard/connectors>`_.
    Note that when a Fivetran sync job is controlled via an Operator, it is no longer
    run on the schedule as managed by Fivetran. In other words, it is now scheduled only
    ...
    def execute(self, context):
        hook = self._get_hook()
        hook.prep_connector(self.connector_id, self.schedule_type)
        return hook.start_fivetran_resync(self.connector_id)
